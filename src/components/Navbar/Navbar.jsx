...
export const Navbar = () => {
  ...
  return (
    <div className="bg-white shadow-md font-lato">
      <div className="flex items-center justify-between px-6 h-[20vw] md:h-auto">
        ...
        >
          <div className="space-y-2">
            <span className={`block h-0.5 bg-black rounded-full transition-all duration-300 ${isOpen ? "w-8 rotate-45 translate-y-2.5" : "w-8"}`} />
            <span className={`block h-0.5 bg-black rounded-full transition-all duration-300 ${isOpen ? "opacity-0" : "opacity-100"}`} />
            <span className={`block h-0.5 bg-black rounded-full transition-all duration-300 ${isOpen ? "w-8 -rotate-45 -translate-y-2.5" : "w-8"}`} />
          </div>
        </button>
        ...
        <div>
          <Link href={"/"} className="px-3 hover:text-[#4C78E7] py-1 text-[1.1rem]">Home</Link>
          <Link href={"/pre-quiz"} className="px-3 hover:text-[#4C78E7] py-1 text-[1.1rem]">Take Quiz!</Link>
          <Link href={"/choose-grade-level"} className="px-3 hover:text-[#4C78E7] py-1 text-[1.1rem]">Explore Careers</Link>
        </div>
        ...
        {isLoggedIn ? (
          <div className="hidden md:flex px-6 relative">
            ...
            >
              <span>{isAdmin.current ? "Admin" : "Profile"}</span>
              <span className="ml-2">
                {isDropdownOpen ? (
                <svg  width="18" height="12" viewBox="0 0 18 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M9 4.2L2.1 11.1L0 9L9 0L18 9L15.9 11.1L9 4.2Z" fill="white"/>
                </svg>
                ) : (
                <svg width="18" height="12" viewBox="0 0 18 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M9 7.8L15.9 0.9L18 3L9 12L0 3L2.1 0.9L9 7.8Z" fill="white"/>
                </svg>
               )}
              </span>
            </button>
            ...
            {isDropdownOpen && (
              <div className="absolute left-6 top-[100%] -mt-[1px] w-[180px] bg-white border border-[#1A6567] rounded-b-[10px] shadow-md">
                <div className="flex flex-col">
                  {isAdmin.current ? (
                    <button
                      className="flex items-center text-black text-[18px] px-4 py-3 hover:bg-gray-100"
                      onClick={() => {
                        router.push("/admin");
                    ...
                    >
                      ...
                      <path d="M10 6V0H18V6H10ZM0 10V0H8V10H0ZM10 18V8H18V18H10ZM0 18V12H8V18H0ZM2 8H6V2H2V8ZM12 16H16V10H12V16ZM12 4H16V2H12V4ZM2 16H6V14H2V16Z" fill="#1C1B1F"/>
                      </svg>
                  ...
                  >
                    Logout
                    <svg className="ml-auto" width="18" height="18" viewBox="0 0 18 18">
                      <path d="M2 18C1.45 18 0.979 17.804 0.588 17.413 0.197 17.022 0 16.551 0 16V2C0 1.45 0.197 0.979 0.588 0.588 0.98 0.197 1.45 0 2 0h7v2H2v14h7v2H2zm11-4l-1.375-1.45L14.175 10H6V8h8.175L11.625 5.45 13 4l5 5-5 5z" fill="#E74C4C" />
                    </svg>
                  </button>
                </div>
              </div>
      ...
      {/* Mobile Menu */}
      <div className={`absolute z-0 left-0 w-full bg-white shadow-md flex flex-col items-center transition-transform duration-300 md:hidden p-6 ${isOpen ? "translate-y-8" : "-translate-y-[200%]"}`}>
        <Link href="/" onClick={() => setIsOpen(false)} className="text-lg text-black hover:text-blue-600 p-2">Home</Link>
        <Link href="/take-quiz" onClick={() => setIsOpen(false)} className="text-lg text-black hover:text-blue-600 p-2">Take Quiz!</Link>
        <Link href="/choose-grade-level" onClick={() => setIsOpen(false)} className="text-lg text-black hover:text-blue-600 p-2">Explore Careers</Link>
        {isAdmin.current && (
          <Link href="/admin" onClick={() => setIsOpen(false)} className="text-lg text-black hover:text-blue-600 p-2 font-semibold">Admin</Link>
        )}
        {isLoggedIn ? (
          <Link href="/results" onClick={() => setIsOpen(false)} className="text-lg text-black bg-[#4C78E757] px-5 py-2 rounded-full hover:text-white m-2">Profile</Link>
        ) : (
          <Link href="/login" onClick={() => setIsOpen(false)} className="text-lg text-black bg-[#4C78E757] px-5 py-2 rounded-full hover:text-white m-2">Log In</Link>
        )}
      </div>
    </div>
  );
